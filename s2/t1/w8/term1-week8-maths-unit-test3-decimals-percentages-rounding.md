# Secondary 2 · Term 1 · Mathematics — Unit Test 3: Decimals, Percentages & Rounding

**Stage 9 · Unit 3**
**29 questions · 40 marks**

This document contains all questions from the interactive sheet in a printable format, plus extra extended-answer questions at the end. Students write their answers directly in their notebooks.

---

## Section A: Multiplying and Dividing by Powers of 10

**Method:** Write 10², 10³, … as 100, 1000, … and 10⁻¹, 10⁻², … as 1/10, 1/100, … Multiplying by a negative power of 10 is the same as dividing by the positive power; dividing by a negative power is the same as multiplying by the positive power.

**Example:** 0.45 ÷ 10⁻³ — dividing by 10⁻³ (a thousandth) is the same as multiplying by 1000 → 0.45 × 1000 = **450**

### Questions

**Q1** (1 mark)
Work out 4.5 × 10²

**Q2** (1 mark)
Work out 7 × 10⁻²

**Q3** (1 mark)
Work out 320 ÷ 10³

**Q4** (1 mark)
Work out 0.06 × 10⁴

**Q5** (1 mark)
When you multiply a positive number by 10 raised to a *negative* power, the result is always … than the number you started with. Write one word.

**Q6** (2 marks)
Work out 8.5 ÷ 10⁻²

**Q7** (2 marks)
Work out 42 ÷ 10⁻¹

---

## Section B: Multiplying and Dividing Decimals

**Method:** Ignore the decimal points, work out the calculation with whole numbers, then place the decimal point back using place value. For division, multiply the top and bottom of the calculation by 10, 100, … to make an equivalent calculation with whole numbers, which is easier to do.

**Example:** −16 ÷ 0.4 → multiply top and bottom by 10 → −160 ÷ 4 = **−40**

### Questions

**Q8** (1 mark)
Work out 12 × 0.4

**Q9** (1 mark)
Work out 0.3 × 0.15

**Q10** (1 mark)
Work out 0.8 ÷ 0.02

**Q11** (2 marks)
Work out −16 ÷ 0.4

**Q12** (1 mark)
Work out 4.5 ÷ 0.5

**Q13** (2 marks)
Work out (36 × 0.5) ÷ (0.2 × 4.5)

**Q14** (1 mark)
0.23 × 37.8 is closest to which of these values?
- A) 0.8
- B) 8
- C) 80
- D) 800

---

## Section C: Compound Percentages

**Method:** Turn each percentage change into a multiplier (add the percentage to 100% for an increase, subtract it from 100% for a decrease, then divide by 100), and apply the multipliers one after another, in order. Repeating the same multiplier n times can be written using a power: ×(multiplier)ⁿ.

**Example:** $12000 decreased by 20%, then by a further 15% → 12000 × 0.8 × 0.85 = **$8160**

### Questions

**Q15** (2 marks)
A price of $250 is increased by 20%, then the new price is decreased by 10%. Work out the final price, in dollars.

**Q16** (1 mark)
Write the multiplier that represents a 15% decrease.

**Q17** (2 marks)
$400 is increased by 10%, then increased by a further 10%. Work out the final value, in dollars.

**Q18** (1 mark)
A value is increased by 20%, then decreased by 20%. Does it end up *higher*, *lower*, or the *same* as the original value? Write one word.

**Q19** (2 marks)
$2000 is invested at 5% interest per year, compounded annually. Work out the value of the investment after 3 years, to the nearest dollar.

**Q20** (1 mark)
Which calculation gives the value of $600 after *two* successive 8% decreases?
- A) 600 × 0.84
- B) 600 × 1.08 × 1.08
- C) 600 × 0.92 × 0.92
- D) 600 × 0.16

**Q21** (2 marks)
A car bought for $18000 loses 15% of its value in the first year, then loses a further 10% of its *new* value in the second year. Work out its value at the end of the second year, in dollars.

**Q22** (1 mark)
A town's population is 8000, and is predicted to fall by 5% each year. Which calculation gives the population after 4 years?
- A) 8000 × 1.05⁴
- B) 8000 × (1 − 0.05 × 4)
- C) 8000 × 0.95 × 4
- D) 8000 × 0.95⁴

---

## Section D: Upper and Lower Bounds

**Method:** A rounded measurement could have come from a range of exact values. The lower bound is half a unit of rounding below the rounded value; the upper bound is half a unit of rounding above it. The range is written as an inequality: lower bound ≤ exact value < upper bound.

**Example:** A crowd of 104000, correct to the nearest 1000 → lower bound = 103500, upper bound = **104500**, so 103500 ≤ x < 104500.

### Questions

**Q23** (1 mark)
A length is measured as 40 cm, correct to the nearest cm. Write the lower bound of the length.

**Q24** (1 mark)
Using the same length (40 cm, correct to the nearest cm), write the upper bound of the length.

**Q25** (1 mark)
A number is rounded to the nearest 100. The answer is 300. Which inequality correctly shows the range of the original number?
- A) 250 ≤ x < 350
- B) 250 ≤ x < 300
- C) 295 ≤ x < 305
- D) 200 ≤ x < 400

**Q26** (2 marks)
The mass of a bag of rice is given as 2.4 kg, correct to 1 decimal place. Write the upper bound of the mass.

**Q27** (2 marks)
The area of a field is given as 1560 m², correct to the nearest 10 m². Which inequality shows the range of possible exact areas?
- A) 1550 ≤ x < 1570
- B) 1555 ≤ x < 1565
- C) 1559.5 ≤ x < 1560.5
- D) 1560 ≤ x < 1570

**Q28** (2 marks)
A stack of 50 identical coins has a height of 40 mm, correct to the nearest mm. Using the lower bound of the stack's height, work out the smallest possible height of a single coin, in mm.

**Q29** (1 mark)
A decimal number is rounded to the nearest whole number, giving 12. Write the correct upper bound.

---

## Part 5: Extended Answer

*These five questions are not on the interactive sheet and are not auto-marked. Write full sentences and show all working in your notebook, and check your answers with your teacher.*

**Q30** (3 marks)
Explain, using an example of your own, why dividing a number by a decimal between 0 and 1 gives an answer larger than the number you started with.

**Q31** (4 marks)
A furniture shop increases the price of a $150 chair by 25% for a display, then later advertises a 25% "loyalty discount" off that new price. Show working to find the final price, and explain why it is not back to $150.

**Q32** (4 marks)
A savings account holds $3000, and interest of 6% is added at the end of each year. Write a single calculation, using a power, that gives the balance after 10 years. Then explain what each part of your calculation represents.

**Q33** (3 marks)
A rectangular field has sides given correct to the nearest metre: 24 m and 15 m. Write the lower and upper bounds for each side, then explain why we cannot say the field's area is exactly 360 m².

**Q34** (4 marks)
Describe, in your own words, the general rule for finding the lower and upper bounds of a number that has been rounded to a given degree of accuracy. Use an example of your own to illustrate your rule.

---

## Mark Breakdown

| Section | Topic | Questions | Marks |
|---------|-------|-----------|-------|
| A | Multiplying and Dividing by Powers of 10 | Q1–Q7 | 9 |
| B | Multiplying and Dividing Decimals | Q8–Q14 | 9 |
| C | Compound Percentages | Q15–Q22 | 12 |
| D | Upper and Lower Bounds | Q23–Q29 | 10 |
| **Interactive total** | | **29** | **40** |
| Extended Answer | Q30–Q34 (printable only) | Q30–Q34 | 18 |

---

## Answer Space

Leave blank space below each question for students to write their working and final answers.

---

*Note: Q1–Q29 match the interactive sheet's questions exactly, so they can be self-checked there. Q30–Q34 are extra extended-answer questions for written practice and are not auto-marked.*

*Source: Interactive version at `s2/t1/w8/term1-week8-maths-unit-test3-decimals-percentages-rounding.html`*
